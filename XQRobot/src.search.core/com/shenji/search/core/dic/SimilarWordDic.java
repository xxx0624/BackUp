...
import com.shenji.robot.action.ConnectionPool;
...
public class SimilarWordDic implements SimilarWordEngine {
	...
	public double getSimilar(String word1, String word2) {
		...
		String wordCodeStr1 = this.getWordCode(word1);
		String wordCodeStr2 = this.getWordCode(word2);
		// 词不存在
	...
	private double getSimilarByCode(String wordCode1, String wordCode2) {
		...
		if (ch1[1] != ch2[1]) {
			...
			n = this.treeBranchNum(String.valueOf(ch1[0]), 1);
		...
		if (!s1.endsWith(s2)) {
			...
			n = this.treeBranchNum(wordCode1.substring(0, 2), 2);
			similar = b * Math.cos(n * PI / 180) * ((double) (n - k + 1) / n);
			return similar;
		}
		if (ch1[4] != ch2[4]) {
			k = Math.abs(ch1[4] - ch2[4]);
			n = this.treeBranchNum(wordCode1.substring(0, 4), 1);
		...
		if (!s1.endsWith(s2)) {
			k = Math.abs(Integer.valueOf(s1) - Integer.valueOf(s2));
			n = this.treeBranchNum(wordCode1.substring(0, 5), 2);
	...
	private void initCodeDB(String path) {
		...
		try {
			connection = ConnectionPool.getInstance().getConnection();
			List<String> codeList = FileUtil.read(path + "同义词编码.txt");
	...
	private void initWordDB(String path) {
		...
		PreparedStatement statement_insert = null;
		try {
			List<String> codeList =FileUtil.read(path + "同义词词林.txt");
	...
	public static void main(String[] args) throws EngineException {
		...
		// for (int i = 0; i < 100; i++) {
		System.out
				.println("Similar----------:" + engine.getSimilar("中国", "宁夏"));
		// }
