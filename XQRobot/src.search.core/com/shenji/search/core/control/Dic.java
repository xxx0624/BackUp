...
public class Dic {
	// 全局锁（词典等文件互斥操作）
	private static final Object LOCK = Dic.class;
	...
	public int addNewSynonmWord(String word, String[] synonmWords) {
		String fc = new Fenci().iKAnalysis(word);
		...
		for (int i = 0; i < synonmWords.length; i++) {
			...
			String syfc = new Fenci().iKAnalysis(synonmWords[i]);
	...
	public int modifySynonmWord(String word, String[] newSynonmWords) {
		...
		for (int i = 0; i < newSynonmWords.length; i++) {
			...
			String syfc = new Fenci().iKAnalysis(newSynonmWords[i]);
			String[] syfcs = syfc.split("/");
			boolean b = false;
			for (String s : syfcs) {
	...
	public String getNowMyWord(String word) {
		...
		String result = new Fenci().iKAnalysis(word);
