...
/*package com.shenji.search.control;
 ...
 import com.shenji.search.old.AllSearch;
 ...
 */
/*
 * public int addNewDialogue(String str, String sessionid) { QAControl control =
 * new QAControl(); int b = control.addNewDialogue(sessionid, str);
 * control.close(); return b; }
 * 
 * // 获得常用语 public String getPhrase(String Question) { Question =
 * Question.trim(); DBPhraseManager dbManager = new DBPhraseManager(); try {
 * return dbManager.getAnswer(Question); } catch (ConnectionPoolException e) {
 * // TODO Auto-generated catch block
 * Log.getLogger(this.getClass()).error(e.getMessage(),e); return null; } }
 * 
 * // 增加常用语 public int addPhrase(String Question, String Answer) { Question =
 * Question.trim(); Answer = Answer.trim(); DBPhraseManager dbManager = new
 * DBPhraseManager(); try { return dbManager.addQA(Question, Answer); } catch
 * (ConnectionPoolException e) { // TODO Auto-generated catch block
 * Log.getLogger(this.getClass()).error(e.getMessage(),e); return -1; } }
 * 
 * // 删除常用语 public int delPhrase(String Question) { Question = Question.trim();
 * DBPhraseManager dbManager = new DBPhraseManager(); try { return
 * dbManager.delQA(Question); } catch (ConnectionPoolException e) { // TODO
 * Auto-generated catch block
 * Log.getLogger(this.getClass()).error(e.getMessage(),e); return -1; } }
 * 
 * // 修改常用语 public int modifyPhrase(String Question, String Answer) { Question =
 * Question.trim(); Answer = Answer.trim(); DBPhraseManager dbManager = new
 * DBPhraseManager(); try { return dbManager.modifyQA(Question, Answer); } catch
 * (ConnectionPoolException e) { // TODO Auto-generated catch block
 * Log.getLogger(this.getClass()).error(e.getMessage(),e); return -1; } }
 * 
 * public String[][] listAllPhrase() { HashMap<String, String> qaMap = null;
 * DBPhraseManager dbManager = new DBPhraseManager(); try { qaMap =
 * (HashMap<String, String>) dbManager.listAllQA(); } catch
 * (ConnectionPoolException e) { // TODO Auto-generated catch block
 * Log.getLogger(this.getClass()).error(e.getMessage(),e); return null; }
 * Iterator<Map.Entry<String, String>> iterator = qaMap.entrySet() .iterator();
 * String[][] reStrs = new String[qaMap.size()][2]; int count = 0; while
 * (iterator.hasNext()) { Map.Entry<String, String> map = iterator.next();
 * reStrs[count][0] = map.getKey(); reStrs[count][1] = map.getValue(); count++;
 * } return reStrs; }
 * 
 * }
 */